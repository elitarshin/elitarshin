import { transformExpressionWithStyles, createTransformerMacro, getSourceMap, addImport } from './utils';
...
export const transformCsslessArrayExpression = ({
  ...
  let sourceMap = state.emotionSourceMap && path.node.loc !== undefined ? getSourceMap(path.node.loc.start, state) : '';
...
let globalTransformer = ({
  ...
  const stylesPropPath = reference.parentPath.get('attributes').find(p => t.isJSXAttribute(p.node) && p.node.name.name === 'styles');
  if (!stylesPropPath) {
    return;
  }
