...
const getKnownProperties = (t, node) => new Set(node.properties.filter(n => t.isObjectProperty(n) && !n.computed).map(n => t.isIdentifier(n.key) ? n.key.name : n.key.value));
const createObjectSpreadLike = (t, file, ...objs) => t.callExpression(file.addHelper('extends'), [t.objectExpression([]), ...objs]);
export let getStyledOptions = (t, path, state) => {
  ...
  let knownProperties = optionsArgument && t.isObjectExpression(optionsArgument) ? getKnownProperties(t, optionsArgument) : new Set();
  if (!knownProperties.has('target')) {
    prodProperties.push(t.objectProperty(t.identifier('target'), t.stringLiteral(getTargetClassName(state, t))));
  }
  let label = autoLabel !== 'never' && !knownProperties.has('label') ? getLabelFromPath(path, state, t) : null;
  if (label) {
