...
import coreMacro, { transformers as coreTransformers, transformCsslessArrayExpression, transformCsslessObjectExpression } from './core-macro';
...
export default function (babel, options) {
  if (options.autoLabel !== undefined && !AUTO_LABEL_VALUES.includes(options.autoLabel)) {
    throw new Error(`The 'autoLabel' option must be undefined, or one of the following: ${AUTO_LABEL_VALUES.map(s => `"${s}"`).join(', ')}`);
  }
  let t = babel.types;
  return {
    name: '@emotion',
      ...
      Program(path, state) {
        ...
        Object.keys(state.opts.importMap || {}).forEach(importSource => {
          ...
          Object.keys(value).forEach(localExportName => {
            ...
            let [exportTransformer, defaultOptions] = Array.isArray(packageTransformers[exportName]) ? packageTransformers[exportName] : [packageTransformers[exportName]];
            transformers[localExportName] = [exportTransformer, {
              ...defaultOptions,
              ...extraOptions,
              ...options
            }];
        ...
        for (const node of path.node.body) {
          if (t.isImportDeclaration(node)) {
            let jsxReactImport = jsxReactImports.find(thing => node.source.value === thing.importSource && node.specifiers.some(x => t.isImportSpecifier(x) && x.imported.name === thing.export));
            if (jsxReactImport) {
